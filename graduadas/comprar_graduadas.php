...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO graduadas (persona, numero_serie, color, tratamiento, cliente_id, precio)
            VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $connection->prepare($sql);
    $stmt->bind_param("ssssss", $persona, $numero_serie, $color, $tratamiento, $cliente_id, $precio);

    if ($stmt->execute()) {
        echo "Gafas graduadas registradas correctamente";
        header("Location: ../graduadas/graduadas.php");
        exit();
    } else {
        echo "Error al registrar las gafas graduadas: " . $stmt->error;
    }
